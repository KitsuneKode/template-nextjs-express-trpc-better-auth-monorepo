...
npx create-kitsu-stack ../test-kitsu-app-fixed --yes
...
export function buildServerEnv(config: ProjectConfig): string {
  const lines: string[] = [
    `# Server environment - generated by create-kitsu-stack`,
...
export function buildWebEnv(): string {
  return `# Frontend environment - generated by create-kitsu-stack
...
npx create-kitsu-stack /tmp/test-app-fixed --yes
...
npx create-kitsu-stack /tmp/test-t3env --yes
cd /tmp/test-t3env
bun install
