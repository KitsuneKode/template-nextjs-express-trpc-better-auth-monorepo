...
export function ArchitectureMap() {
  return (
    <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {architectureGroups.map((group) => (
            <section key={group.column}>
              <p className="mb-3 text-xs font-semibold tracking-[0.16em] text-[#cfaf87] uppercase">{group.column}</p>
              <div className="space-y-3">
                {group.items.map((item) => (
                  <article key={item.name} className="rounded-xl border border-white/8 bg-[#090d12] p-3">
      ...
      <div className="rounded-3xl border border-white/10 bg-[#0d1218]/86 p-5">
        <p className="mb-3 text-xs font-semibold tracking-[0.16em] text-[#cfaf87] uppercase">Request flow</p>
