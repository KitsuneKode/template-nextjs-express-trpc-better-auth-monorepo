...
This guide was generated by \`create-kitsu-stack\` for ${options.projectName}.
...
export async function scaffoldProject(options: BootstrapOptions): Promise<ScaffoldResult> {
  ...
  runCommand(['bun', 'toolings/scripts/rename-scope.ts', '--quiet'], destinationDir)
  ...
  if (options.initializeGit) {
    try {
      runCommand(['git', 'init', '-b', 'main'], destinationDir)
    } catch {
      // Keep bootstrap usable even if git is not installed in the environment.
    }
  }

  if (options.installDependencies) {
    runCommand(['bun', 'install'], destinationDir)
  }
